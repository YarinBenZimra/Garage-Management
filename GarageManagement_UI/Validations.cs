...
namespace GarageManagement_UI
{
    public class Validations
    {
        ...
        {
            int counter = 1, vehicleType = 0;

            Console.WriteLine(string.Format("Enter your vehicle type, please choose one of the following options:"));

            foreach (string vehicleTypeName in i_VehiclesTypeNames)
            {
                Console.WriteLine(string.Format("{0}. {1}", counter++, vehicleTypeName));
            }
        ...
        public static void GetWheelsDetails(ref string io_ManufacturerName, ref float io_CurrentAirPressure, float i_MaxAirPressure)
        {
            ...
            Console.WriteLine(string.Format("Please enter wheel's manufacturer name:"));
            io_ManufacturerName = Console.ReadLine();
            Console.WriteLine();

            Console.WriteLine(string.Format("Enter the current air pressure: (Maximum: {0})", i_MaxAirPressure));
        ...
        {
            Console.WriteLine(string.Format("Please enter the owner's name:"));
            io_OwnerName = Console.ReadLine();
            Console.WriteLine();

            CheckOwnerNameValidation(io_OwnerName);

            Console.WriteLine(string.Format("Please enter the owner's phone number:"));
        ...
        public static bool AskIfDisplayByState()
        {
            int input = 0;
            bool answer = false;
            Console.WriteLine(string.Format(@"Do you want to display vehicle by their state?
1. Yes
2. No"));
        ...
        {
            int state = 0;
            Console.WriteLine(string.Format(@"Please enter the vehicle state, choose one of the following states: 
1. Under Repair
2. Repaired
3. Paid"));
        ...
        public static void CheckPhoneNumberValidation(string i_PhoneNumber)
        {
            ...
            if (digitsOnly.Length == i_PhoneNumber.Length)
            {
                if (digitsOnly.Length == 10)
                {
                    if (digitsOnly[0] != '0')
                    {
                        throw new FormatException(k_InvalidPhoneNumberMessage);
                    }
                }
                else
                {
        ...
        public static void CheckOwnerNameValidation(string i_Name)
        {
            foreach (char c in i_Name)
            {
                ...
                {
                    throw new FormatException(k_InvalidNameMessage);
                }
            }
        }
        ...
        public static void CheckLicenseNumberValidation(string i_LicenseNumber)
        {
            ...
            if (digitsOnly.Length == i_LicenseNumber.Length)
            {
                ...
                {
                    throw new FormatException(k_InvalidLicenseNumberMessage);
                }
            }
